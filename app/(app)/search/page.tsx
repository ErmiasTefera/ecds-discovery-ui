...
export default function SearchPage() {
  ...
  const [advancedFilters, setAdvancedFilters] = useAtom(
    advancedSearchFiltersAtom
  );
  ...
  useEffect(() => {
    ...
    const advancedPresent = Boolean(
      criteriaParam || filtersParam || facetsParam
    );
    ...
    if (facetsParam) {
      try {
        const parsed = JSON.parse(decodeURIComponent(facetsParam)) as Record<
          string,
          string[]
        >;
      ...
      try {
        ...
        if (advancedFilters.format !== "all") {
          results = results.filter(
            (result) => result.type === advancedFilters.format
          );
        }
        ...
        if (advancedFilters.yearFrom || advancedFilters.yearTo) {
          results = results.filter((result) => {
            ...
            const fromYear = advancedFilters.yearFrom
              ? parseInt(advancedFilters.yearFrom)
              : 1900;
            const toYear = advancedFilters.yearTo
              ? parseInt(advancedFilters.yearTo)
              : 2030;
          ...
          <>
            <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="flex-1">
                <div className="flex w-full justify-between items-center gap-4 mb-6 mt-3">
                  <SearchInfo />
